...
def test_element_counter():
    ...
    assert my_tracker.element_counter == {'\\lx': 3}
...
def test_number_duplicates_in_list_with_multiple_examples():
    my_list = ['\\lx', 'Arroyo', '\\ge', 'stream', '\\xv', 'the stream', '\\xv', 'the stream flows', '\\ge', 'water', '\\xv', 'the water is blue']
    my_tracker = Tracker({'\\ge', '\\xv'}, '\\ge')
    my_tracker.number_duplicates_in_list(my_list)
    assert my_list == ['\\lx', 'Arroyo', '\\ge1', 'stream', '\\xv1', 'the stream', '\\xv1-2', 'the stream flows', '\\ge2', 'water', '\\xv2', 'the water is blue']